...
class MCI(Function):
    ...
    def forward(ctx, input):
        ...
        w = x.data.shape[3]
        M = h * w
        P = x.reshape(batchSize, dim, M)
        # PPT = P.transpose(1,2).bmm(P)
        # Qtile = PPT.bmm(P.transpose(1,2))
        # Qtile = Qtile.transpose(1,2).reshape(batchSize, dim, h,w)
        PPT = P.bmm(P.transpose(1, 2))
        Qtile = PPT.bmm(P)
        Qtile = Qtile.reshape(batchSize, dim, h, w)
        ctx.save_for_backward(Qtile, P)
        return Qtile
